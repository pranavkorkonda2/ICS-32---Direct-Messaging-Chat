...
class Profile:
    ...
    def save_profile(self, path: str) -> None:
        ...
        # Accept new .dsu file paths (create if does not exist) and ensure valid extension
        if p.suffix != '.dsu':
    ...
    def load_profile(self, path: str) -> None:
        """"Loads user data, contacts, and message history from a .dsu file into the current Profile instance"""
        p = Path(path)
        ...
        if p.exists() and p.suffix == '.dsu':
            try:
                with open(p, 'r') as f:
                    ...
                    for post in data.get('_posts', []):
                        new_post = Post(post['entry'], post['timestamp'])
                        self._posts.append(new_post)
            except json.JSONDecodeError as e:
                print(f"Error decoding DSU file: {e}")
                raise DsuProfileError(e)
            except Exception as e:
...
def cleanup(filename='Profile.py'):
    """Removes all trailing whitespace from file"""
